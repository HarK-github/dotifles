...
playerctl metadata -F -f '{{playerName}}|{{title}}|{{artist}}|{{mpris:artUrl}}|{{status}}|{{mpris:length}}' 2>/dev/null | while read -r line; do
    IFS='|' read -r name title artist artUrl status length <<< "$line"

    if [[ -n "$length" ]]; then
        length_secs=$((length / 1000000))
        # Get duration string separately to avoid complex math in bash
